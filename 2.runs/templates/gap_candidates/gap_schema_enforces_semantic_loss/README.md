# Gap Candidate Run Template: Schema enforces semantic loss

This is a **run template** (not an executed run).

Purpose:
- Attempt to construct an admissible path for: **Schema enforces semantic loss**
- If no admissible path exists, terminate with **STOP + GAP**.

This template must be instantiated into a dated run directory under:
`2.runs/YYYY-MM-DD/run_XX_gap_schema_enforces_semantic_loss/`
