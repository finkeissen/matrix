# Gap Candidate Run Template: Self-application violates constraints

This is a **run template** (not an executed run).

Purpose:
- Attempt to construct an admissible path for: **Self-application violates constraints**
- If no admissible path exists, terminate with **STOP + GAP**.

This template must be instantiated into a dated run directory under:
`2.runs/YYYY-MM-DD/run_XX_gap_self_application_violates_constraints/`
