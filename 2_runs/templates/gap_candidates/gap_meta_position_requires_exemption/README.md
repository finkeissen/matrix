# Gap Candidate Run Template: Meta-position requires exemption

This is a **run template** (not an executed run).

Purpose:
- Attempt to construct an admissible path for: **Meta-position requires exemption**
- If no admissible path exists, terminate with **STOP + GAP**.

This template must be instantiated into a dated run directory under:
`2.runs/YYYY-MM-DD/run_XX_gap_meta_position_requires_exemption/`
