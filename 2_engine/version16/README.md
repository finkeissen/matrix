...
Die wichtigste strukturelle Veränderung in v14: Prompt-Text ist von Step-Logik getrennt.

```
Schicht 1: Prompt-Text    → pipeline/prompts/<step>.md
                             Reiner Text mit {placeholders}. Keine Python-Logik.
                             Versionsgeschichte über git blame allein lesbar.

Schicht 2: Step-Logik     → pipeline/steps/step_<n>.py
                             Enthält: validate_output(), run(),
                             Novelty Guard, Artifact-IO, Telemetrie-Aufrufe.
                             Kein Prompt-Text. Lädt Prompt via load_prompt().

Schicht 3: Infrastruktur  → common.py, prompt_loader.py, telemetry.py, validator.py
                             Step-unabhängig. Wird von allen Steps importiert.
```

**Was das bedeutet:**
- Prompt-Verbesserungen berühren ausschließlich `prompts/<step>.md`
- Step-Refactorings berühren nie den Prompt-Text
- Beide Dimensionen haben eigene Git-History und sind unabhängig reviewbar
- A/B-Tests zwischen Prompts sind mit `--prompt-variant` ohne Code-Änderung möglich

**Was v14 noch nicht löst:** `validate_output()` lebt weiterhin in der Step-Datei.
Eine Auslagerung als maschinenlesbare Kontrakt-Datei (`prompts/<step>.contract.json`)
ist der logische nächste Schritt — in v14 bewusst noch nicht umgesetzt.
...
│
├── steps/                       ← run(), validate_output(), Novelty Guard, IO
│   ├── step_01_scope.py
