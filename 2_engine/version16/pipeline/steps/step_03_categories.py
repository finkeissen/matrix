"""
step_03_categories.py v14 — Identify thematic clusters.
Type: LLM | Model: 35b | Prompt: prompts/03_categories.md
"""
...
STEP_KEY = "03_categories"


def validate_output(obj: dict) -> list[str]:
    errors = []
    for field in ["subdomain_id", "items", "category_count"]:
        if field not in obj:
    ...
    if "items" in obj:
        ...
        for item in obj["items"]:
            if "name" not in item or "description" not in item:
                errors.append(f"item missing name or description: {item.get('name','?')}")
            epc = item.get("estimated_problem_count", 0)
            if not (5 <= epc <= 40):
                errors.append(f"estimated_problem_count={epc} out of range 5–40 for '{item.get('name','?')}'")
            name = item.get("name", "")
            if name.lower() in ("miscellaneous", "other", "general"):
                errors.append(f"category name '{name}' is too vague")
    if "items" in obj and "category_count" in obj:
        if obj["category_count"] != len(obj["items"]):
            errors.append(f"category_count={obj['category_count']} != len(items)={len(obj['items'])}")
    return errors


def run(run_id: str, scope: dict, scope_hash: str,
        structure: dict, structure_hash: str, kb_snapshot_id: str,
        tel=None) -> dict:
    ...
    if novelty_guard_check(WORK_DIR, run_id, task_id) and out_path.exists():
        ...
        return {"status": "ok", "categories": cats, "categories_hash": sha256_file(out_path)}
    ...
    if tel:
        ...
        tel.record_content(STEP_KEY, {
            "category_count": len(cats.get("items", [])),
            "avg_estimated_count": (
                sum(i.get("estimated_problem_count", 0) for i in cats.get("items", []))
                / max(1, len(cats.get("items", [])))
            ),
        })

    write_json(out_path, cats)
    cats_hash = register_artifact(WORK_DIR, run_id, f"{STEP}:categories",
    ...
    emit_state(WORK_DIR, run_id, "step.done", STEP_KEY,
               {"task_id": task_id, "category_count": len(cats.get("items", [])),
                "prompt_hash": prompt_hash})

    print(f"  [done] {len(cats.get('items', []))} categories")
