...
from constants import SYSTEM_VERSION, PIPELINE_VERSION, ContentState, ReviewStatus, make_created_by, ArtifactKey
from common import (
    llm_call, sha256_file, now_iso,
    artifact_dir, write_json, write_jsonl_line, register_artifact,
    emit_state, create_snapshot,
)
from validator import make_problem_uid

STEP = "08_finalization"

STEP_KEY_SUMMARY = "08_finalization_summary"
...
def run(run_id: str, scope: dict, scope_hash: str,
    ...
    created_by   = make_created_by(PIPELINE_VERSION, run_id)
    created_at   = now_iso()
    ...
    for rev in all_reviewed:
        ...
        for p in rev.get("problems", []):
            ...
            record = {
                ...
                "problem_uid":      make_problem_uid(p, subdomain_id, cat_name),
                ...
                "review_status":    ReviewStatus.REVIEWED,   # promoted from draft by 05_validation
                "content_state_at_commit": ContentState.VERIFIED,
            }
    ...
    register_artifact(WORK_DIR, run_id, ArtifactKey.FINAL_PROBLEMS,
                      final_path, content_state=ContentState.VERIFIED, step=STEP)

    # ── LLM run summary (graceful degradation) ────────────────────────────────
    from common import load_run_record
    rec = load_run_record(WORK_DIR, run_id)

    summary_prompt_text, summary_prompt_meta = load_prompt(STEP_KEY_SUMMARY)
    summary_prompt = summary_prompt_text.format(
        subdomain_label=subdomain_label,
    ...
    raw_summary = llm_call(summary_prompt, retries=0, tel=tel,
                           step=STEP_KEY_SUMMARY,
                           prompt_hash=summary_prompt_meta["hash"],
                           model_class="19b")
    ...
    register_artifact(WORK_DIR, run_id, ArtifactKey.RUN_AUDIT,
                      audit_path, content_state=ContentState.VERIFIED, step=STEP)
