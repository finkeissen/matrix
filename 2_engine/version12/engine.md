...
Efficient & Effective structures & processes storing each major step & explanations & logs for later evaluations.
...
|---|---|---|
| Now | 19B/A3B (65 T/s), 35B/A3B (30 T/s), 122B/A10B (3 T/s) | MoE models fit VRAM efficiently; 122B via CPU offload |
...
*/15 * * * * rsync -a --delete /home/ef/ram/runs/ /home/ef/Beruflich/GitHub/3_matrix_artifacts/2_engine/version10/data/runs/

**2. Claude Code bekommt im Briefing-Zusatz diese Pfade:**
INPUT    = /home/ef/Beruflich/GitHub/3_matrix_artifacts/2_engine/version10/
           # read-only — engine, schemas, pipeline, seeds
WORK_DIR = /home/ef/ram/runs/
           # RAM-Disk — laufende runs, flüchtig, rsync-Ziel alle 15min
OUTPUT   = /home/ef/Beruflich/GitHub/3_matrix_artifacts/2_engine/version11/data/registry/
           # SSD — 09_commit schreibt hier atomar (problems.jsonl, run_log.jsonl)
ARCHIVE  = /home/ef/Beruflich/GitHub/3_matrix_artifacts/2_engine/version11/data/runs/
           # SSD — rsync-Ziel für WORK_DIR (systemd-Timer, alle 15min)
...
# /etc/systemd/system/ram-sync.service
[Unit]
Description=RAM-Disk Run Sync

[Service]
Type=oneshot
ExecStart=rsync -a --delete /home/ef/ram/runs/ /home/ef/Beruflich/GitHub/3_matrix_artifacts/2_engine/version11/data/runs/

# /etc/systemd/system/ram-sync.timer
[Unit]
Description=RAM-Disk Run Sync alle 30min

[Timer]
OnBootSec=30min
OnUnitActiveSec=30min
Unit=ram-sync.service

[Install]
WantedBy=timers.target


## Efficiency & Hygiene Rules (Slim System)

### 1. Code-Refactoring & Cleanup
- **No Dead Code:** Identifiziere bei jeder Änderung ungenutzte Variablen, Imports oder veraltete Funktionen und schlage deren Entfernung vor.
- **DRY Principle:** Wenn du Codeduplikate findest, schlage vor, diese in eine zentrale Utility-Funktion zu überführen.
- **Dependency Check:** Bevor du neue Pakete vorschlägst, prüfe, ob die Aufgabe mit vorhandenen Bordmitteln oder schlankeren Alternativen gelöst werden kann.

### 2. File System Hygiene
- **Temporary Files:** Erstelle keine temporären Dateien oder Backups im Projektordner, die nicht automatisch von .gitignore erfasst werden.
- **Deletions:** Wenn eine Datei durch ein Refactoring obsolet wird, frage im Plan explizit nach deren Löschung ("Soll ich [Datei] entfernen?").

### 3. Build & Performance
- **Modern Standards:** Nutze bevorzugt moderne Sprachfeatures (z.B. native Browser-APIs statt schwerer Libraries), um das Bundle-Gewicht niedrig zu halten.
- **Linting:** Achte strikt darauf, dass durch Änderungen keine neuen Linting-Warnungen entstehen.
