...
Problems in `final_problems.jsonl` carry state `verified` (promoted by `05_validation`). No state transition occurs here. `09_commit` records the commit and writes the final snapshot — it does not change artifact state.
