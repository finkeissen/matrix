...
def run(run_id: str, categories: dict, categories_hash: str, kb_snapshot_id: str, tel=None) -> dict:
    """
    ...
    schema_errs = validate_schema(result, "normalized_categories")
    if schema_errs:
        emit_state(WORK_DIR, run_id, "step.stop", STEP,
                   {"stop_code": "schema_validation_failed", "errors": schema_errs})
        return {"status": "stop", "stop_code": "schema_validation_failed"}
