...
def parse_lmstudio_json(content: str) -> List[Dict[str, Any]]:
    ...
    # 2) Extract first balanced JSON fragment
    fragment = extract_balanced_json_fragment(text)
    if fragment is None:
        raise ValueError(f"Could not find balanced JSON in model response: {text[:500]}")
    ...
    raise ValueError(f"Balanced JSON found, but format is invalid: {fragment[:500]}")
...
def generate_with_lm_studio(
        ...
        "messages": [
            {"role": "system", "content": system_msg},
            {
                ...
                "content": (
                    ...
                    "No markdown. No preamble. No postscript. No explanations. "
                    'Exact format: {"items":[{"subdomain":"...","atomic_problems":["..."]}]}\n'
                    f"Input:\n{user_msg}"
                ),
            },
        ],
    ...
    if not content:
        raise RuntimeError(f"LM Studio returned empty message content. Full response: {raw[:1000]}")
    ...
    # Fill missing outputs deterministically if the model omitted some
