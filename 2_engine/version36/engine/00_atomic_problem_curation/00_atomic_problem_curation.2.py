...
- provider=lm-studio (direkter HTTP-Request an LM Studio, keine openai-Abhängigkeit)
...
def build_prompt(domain: str, subdomains: List[str], atomic_per_subdomain: int) -> Tuple[str, str]:
    system = (
        ...
        'Form: ein JSON-Objekt mit Feld "items", das ein Array von Objekten enthält. '
        'Jedes Objekt hat die Felder {"subdomain": string, "atomic_problems": [string, ...]}. '
...
def generate_with_lm_studio(
    ...
    try:
        ...
        raise RuntimeError(
            f"Unexpected LM Studio response format: {raw[:1000]}"
        ) from exc
