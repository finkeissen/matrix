...
def build_step_input(ctx: RunContext, step_name: str, domain: str) -> dict[str, Any]:
    ...
    """
    assemblers = {
        "00_atomic_problem_curation": _input_00_atomic_problem_curation,
        "01_atomic_problem_merge":    _input_01_atomic_problem_merge,
...
def _input_00_atomic_problem_curation(ctx: RunContext, domain: str) -> dict:
    """00_atomic_problem_curation input: domain + curation config from RunContext.

    provider_config and curation_params are resolved from ctx.manifest.model_config
    so they can be set once at run-creation time and flow through automatically.
    subdomains_file must be provided in model_config or defaults to ingestion path.
    """
    mc = getattr(ctx.manifest, "model_config", {}) or {}
    curation = mc.get("curation", {})
    return {
        "domain": domain,
        "subdomains_file": curation.get(
            "subdomains_file",
            str(ctx.run_dir.parent.parent / "ingestion" / "seeds" / f"{domain}_subdomains.jsonl"),
        ),
        "provider": curation.get("provider", "offline-template"),
        "output_dir": str(ctx.run_dir / "ap_candidates"),
        "provider_config": curation.get("provider_config", {}),
        "curation_params": curation.get("curation_params", {}),
    }


def _input_01_atomic_problem_merge(ctx: RunContext, domain: str) -> dict:
    """01_atomic_problem_merge input: candidate batch files from 00_atomic_problem_curation."""
    curation_out = _read_step_output(ctx, "00_atomic_problem_curation")
    return {
        "domain": domain,
        "input_files": curation_out.get("output_files", []),
        "output_dir": str(ctx.run_dir / "ap_store"),
