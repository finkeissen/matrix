...
STEP_SCHEMA_MAP = {
    ...
    "03_categories": "categories.schema.json",
...
class SchemaValidator:
    ...
    def validate_step_output(self, step_name: str, result: dict) -> list[str]:
        ...
        return self._validate(data, schema, step_name, step_name=step_name)
    ...
    def _validate(
        ...
        *,
        ...
        validate_as_single_object = step_name == "03_categories"

        if validate_as_single_object:
            items = [(None, data)]
        else:
            items = list(enumerate(data)) if isinstance(data, list) else [(0, data)]

        for index, item in items:
            for error in validator.iter_errors(item):
                prefix = f"[{context}]" if index is None else f"[{context}][{index}]"
                json_path = error.json_path or "$"
                errors.append(f"{prefix} {json_path}: {error.message}")
