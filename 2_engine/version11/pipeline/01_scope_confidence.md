...
recommendation = "clarify"   → 01_scope re-run with flagged_ambiguities injected as
                               additional_context (NOT via 06_clarification — no validation_report
                               exists at this stage). Orchestrator increments clarification_rounds.
                               If clarification_rounds >= 2 → STOP: scope_clarification_exhausted
```

**Rationale:** `06_clarification` is triggered by `05_validation` (post-generation scope violations). Early scope problems detected here at `01_scope_confidence` are handled by direct re-entry to `01_scope` with the `flagged_ambiguities` list as structured input — no full validation_report exists yet.
