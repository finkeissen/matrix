...
class Orchestrator:
    ...
    def run(self, domain: str, prompt_variant: str = '', steps: Optional[list[str]] = None, dry_run: bool = False, run_id: Optional[str] = None) -> Optional[str]:
    ...
    def _run_step_with_retry(self, ctx: RunContext, step_name: str, domain: str) -> bool:
        ...
        while retry_count <= self.max_retries:
            try:
                # Write step-local input envelope before execution
                input_payload = {"domain": domain, "step": step_name}
                input_path = str(ctx.write_step_payload(step_name, "input", input_payload))
                ...
                # Write step-local output envelope
                output_data = result.get("data", {})
                output_path_obj = ctx.write_step_payload(step_name, "output", output_data)
                # Write step-local meta envelope
                ctx.write_step_payload(step_name, "meta", {
    ...
    def _load_step(self, step_name: str):
        mod = importlib.import_module(f'pipeline.steps.{step_name}')
        return mod.run
