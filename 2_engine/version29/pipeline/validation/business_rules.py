...
def check_problem_business_rules(problem: dict[str, Any]) -> list[str]:
    ...
    if problem.get("difficulty") and problem["difficulty"] not in {"easy", "medium", "hard", "expert"}:
        errors.append(f"difficulty must be one of easy|medium|hard|expert, got {problem['difficulty']}")
...
def validate_business_rules(step_name: str, data: Any) -> list[str]:
    if data is None:
        return [f"[{step_name}] data is missing"]

    if step_name == "01_scope":
        items = data if isinstance(data, list) else [data]
        errors: list[str] = []
        for i, item in enumerate(items):
            errs = check_scope_business_rules(item)
            errors.extend([f"[{step_name}][{i}] {e}" for e in errs])
        return errors

    if step_name in PROBLEM_STEPS:
        items = data if isinstance(data, list) else [data]
        errors: list[str] = []
        for i, item in enumerate(items):
            errs = check_problem_business_rules(item)
            errors.extend([f"[{step_name}][{i}] {e}" for e in errs])
        return errors

    # 02_seed_expansion, 03_categories, and 08_export are not
    # validated here as atomic problems.
    return []
