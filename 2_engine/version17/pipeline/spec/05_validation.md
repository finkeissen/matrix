...
valid=false AND schema_errors only          → orchestrator retries 04a/04b for affected categories
                                              (increments per-category retry_count; uses rejection
                                               reason as retry_context_hash input to 04a)
valid=false AND atomicity_failure_rate>0.20 → orchestrator retries 04a/04b for affected categories
valid=false AND scope_violations            → 06_clarification
all retries exhausted                       → 08_finalization with pipeline_status: insufficient
```

**Retry trigger responsibility:** `05_validation` writes the `validation_report` only. The orchestrator reads `valid`, `schema_errors`, and `atomicity_failures` and dispatches retries. `05_validation` itself has `retries: 0` — it never re-runs; it is the retry decision point, not the retry executor.
...
Note: `09_commit` does not re-promote content state. `verified` is set here by `05_validation` as the authoritative quality gate. `09_commit` records the commit but does not change artifact state.
